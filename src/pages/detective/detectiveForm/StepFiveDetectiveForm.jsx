...
const StepFiveDetectiveForm = ({
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1"><CreditCard size={18} className="text-[#D92B3A]" /><h3 className="text-base font-semibold">Banking Information</h3></div>
      <p className="text-sm text-gray-400 mb-6">For salary and payment processing</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        <div>
          <label className={labelCls}>Bank Name *</label>
          <input placeholder="Chase Bank" value={data.banking.bankName} onChange={e => handleChange('banking', 'bankName', e.target.value)} className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Account Holder Name *</label>
          <input placeholder="John Doe" value={data.banking.holderName} onChange={e => handleChange('banking', 'holderName', e.target.value)} className={inputCls} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
        <div>
          ...
          <input type="password" placeholder="........" value={data.banking.accountNumber} onChange={e => handleChange('banking', 'accountNumber', e.target.value)} className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Routing Number *</label>
          <input placeholder="123456789" value={data.banking.routingNumber} onChange={e => handleChange('banking', 'routingNumber', e.target.value)} className={inputCls} />
        </div>
      </div>
